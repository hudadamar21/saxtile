    ...
    <template #body>
      ...
      <ul>
        <li class="w-full bg-white m-2 px-3 py-2 shadow rounded flex items-center justify-between" 
          ...
          <p class="text-base" :class="{'text-blue-500': isUrl(collection.prefix + text)}">{{ collection.prefix }}<strong>{{ text }}</strong></p>
          <div class="flex items-center">
            ...
            <a v-if="isUrl(collection.prefix + text)"
              @click.stop
  ...
  methods: {
    ...
    deleteCollection(){
      if(window.confirm(`yakin mau hapus Collection '${this.collection.title}' ? `)){
        this.$store.dispatch('text_collection/Delete', this.collection.id, {root: true})
      }
    },
    closeModal(){
